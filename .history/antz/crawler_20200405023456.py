...
def searchBrandModelLinks():
  ...
  r = requests.get(url)
  soup = BeautifulSoup(r.content, 'html.parser')
  temp_links = soup.find_all('a', class_="desktop-specific-brand-models-link")
  print(f' I got {len(links)} links')
  return links  
...
def getCarLinks(searchLink):
  ...
  while True:
    ...
    print(f'for {brand}, {model}, {trim}, I got {len(links)} up to now')
  ...
  for link in links:
      try:
        if link['href'] != "#":
          ...
          print(f'len is {car_links[0]}')
          
          for item in car_links:
            year, price, kilometer = crawlCarPage(item)  
